...
# 设置工作目录
setwd("E:/9. Chemo_AEs_altas/Analysis/Results/Figure 2/Figure 2F/log2FC")
...
for(file in files) {
  ...
  colnames(data)[1] <- "pathway"  # 将gene改为pathway
  all_data_list[[organ]] <- data
}

# 读取选定的通路列表
selected_pathways <- read_excel("mice_pandrug_pathway.xlsx")
...
calculate_zscores_and_pvalues <- function(target_organ, all_data_list) {
  ...
  results <- data.frame(
    ...
    manual_log2FC = target_data$manual_log2FC
  )
  
  for(pathway in results$pathway) {
    all_manual_log2FCs <- numeric()
    target_manual_log2FC <- target_data$manual_log2FC[target_data$pathway == pathway]
    
    for(organ in names(all_data_list)) {
      organ_data <- all_data_list[[organ]]
      if(pathway %in% organ_data$pathway) {
        all_manual_log2FCs <- c(all_manual_log2FCs, 
                        organ_data$manual_log2FC[organ_data$pathway == pathway])
      }
    }
    
    if(length(all_manual_log2FCs) > 1) {
      mean_manual_log2FC <- mean(all_manual_log2FCs, na.rm = TRUE)
      sd_manual_log2FC <- sd(all_manual_log2FCs, na.rm = TRUE)
      
      if(!is.na(sd_manual_log2FC) && sd_manual_log2FC != 0) {
        results$zscore[results$pathway == pathway] <- 
          (target_manual_log2FC - mean_manual_log2FC) / sd_manual_log2FC
      } else {
        results$zscore[results$pathway == pathway] <- NA
      }
      
      other_manual_log2FCs <- all_manual_log2FCs[all_manual_log2FCs != target_manual_log2FC]
      
      if(length(other_manual_log2FCs) >= 2) {
        tryCatch({
          test_result <- t.test(
            other_manual_log2FCs,
            mu = target_manual_log2FC,
...
# 读取通路名映射文件
pathway_mapping <- read_excel("mice_pandrug_pathway_2.xlsx")
...
clean_pathway_names <- function(pathway) {
  ...
  if(!grepl("^(GO_|REACTOME_|GOBP_|GOCC_|GOMF_)", pathway)) {
    return(pathway)
  }
  
  cleaned <- gsub("^(GO_|REACTOME_|GOBP_|GOCC_|GOMF_)", "", pathway)
...
map_pathway_names <- function(pathway) {
  ...
  if(nrow(matching_row) > 0) {
    ...
    return(clean_pathway_names(pathway))
  }
}
...
# 将pathway_clean转换为factor，设置正确的顺序
...
# 保存图片
ggsave("selected_pathway_zscores.pdf", p, width = 12, height = 5, dpi = 300)
...
# 保存无标签版本
ggsave("selected_pathway_zscores_no_labels.pdf", p_no_labels, width = 12, height = 3, dpi = 300)
...
# 保存带X轴标签版本
ggsave("selected_pathway_zscores_with_x_labels.pdf", p_with_x_labels, width = 12, height = 4.2, dpi = 300)
