...
setwd("E:/9. Chemo_AEs_altas/Analysis/Results/Figure 3/Figure 3E")

# Get all csv files
files <- list.files(pattern = ".*_vs_PBS_results\\.csv$")  # Modified pattern
...
for(file in files) {
  # Modified to extract organ name from new file format
  organ <- gsub("(.+)_vs_PBS_results\\.csv", "\\1", file)  # Changed pattern
